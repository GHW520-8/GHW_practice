...
void patient::show_Myorder() {
	...
	for (int i = 0; i < ord_f.order_size; i++) {
		...
		if (atoi(ord_f.m_save[i]["phone_number"].c_str()) == stoi(this->phone_num)) {
			//预约日期
			std::cout << "星期" << lam::day(stoi(ord_f.m_save[i]["date"])) << "\t\t";
			//预约时间段
			std::cout << (ord_f.m_save[i]["intraval"] == "1" ? "上午" : "下午") << "\t\t";
			//科室
			std::cout << lam::office(stoi(ord_f.m_save[i]["office_num"])) << "\t\t";
			//预约医生
			std::cout << lam::doc_name(stoi(ord_f.m_save[i]["select_doctor"])) << "\t\t";
			//预约状态
			std::cout << lam::status(stoi(ord_f.m_save[i]["status"])) << std::endl << std::endl;
		}
	}
	system("pause");
	system("cls");
...
void patient::cancel_order() {
	...
	mfc::get_doctor_name();
	using namespace lam;
	std::vector<int> cal_temp;
	std::cout << "只有审核中或预约成功的记录才可以取消噢{{{(>_<)}}}\n" << std::endl;
	std::cout << "    预约日期\t\t" << "时间段\t\t" << "科室\t\t" << "医生\t\t" << "状态\n" << std::endl;
	for (int i = 0, j = 1; i < c_order.order_size; i++) {
		if (c_order.m_save[i]["phone_number"] == this->phone_num) {
			if (c_order.m_save[i]["status"] == "1" || c_order.m_save[i]["status"] == "2") {
				//预约日期
				std::cout << "[" << j << "] " << "星期" << day(stoi(c_order.m_save[i]["date"])) << "\t\t";
				//预约时间段
				std::cout << (c_order.m_save[i]["intraval"] == "1" ? "上午" : "下午") << "\t\t";
				//科室
				std::cout << office(stoi(c_order.m_save[i]["office_num"])) << "\t\t";
				//预约医生
				std::cout << doc_name(stoi(c_order.m_save[i]["select_doctor"])) << "\t\t";
				//预约状态
				std::cout << status(stoi(c_order.m_save[i]["status"])) << std::endl << std::endl;
				cal_temp.emplace_back(i);
				j++;
			}
		}
	}
	std::cout << "请输入你要取消的预约记录,输入[0]返回\n" << std::endl;
	int select = 0;
	while (true) {
		std::cin >> select;
		if (select >= 0 && select <= c_order.order_size) {
			if (select == 0) {
					break;
			}
			else {
				c_order.m_save[cal_temp[select]]["status"] = "3";
				c_order.update_order(this->phone_num);
				std::cout << "该预约记录已被取消 ＞﹏＜...\n" << std::endl;
				break;
			}
		}
