...
//有参构造函数
administrator::administrator(std::string admi_name, std::string num, std::string password) {
	this->user_name = admi_name;
	this->administrator_num = num;
...
void administrator::clean_file() {
	std::cout << "真的要清空吗 ＞︿＜ ! ..." << std::endl;
	std::cout << "输入[0]确定，按任意键返回...\n" << std::endl;
	while (true) {
		if (_kbhit()) {
			int key = _getch();
			if (key == 0 || key == 0XE0) {
				std::cout << "下次注意噢 T_T..." << std::endl;
				break;
			}
			if (key == '0') {
